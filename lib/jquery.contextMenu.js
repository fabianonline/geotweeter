...
(function($) {
	...
	var post_context_menu = function(e) {
		...
		if(((options.leftClick && e.button == 0) || (options.leftClick == false && e.button == 2)) && !e.shiftKey){
			e.preventDefault();
			...
			$.each(items, function(index, item) {
				...
				if (item.separator_above) {
					item_html.addClass("separator_above");
				}
				if (item.separator_below) {
					item_html.addClass("separator_below");
				}
	...
	jQuery.fn.contextMenu = function ( name, options ) {
		options = pre_context_menu(this, name, options);
		return me.bind('contextmenu click', options, post_context_menu);
	}

	jQuery.fn.delegateContextMenu = function ( selector, name, options ) {
		options = pre_context_menu(this, name, options);
		return $(document).delegate(selector, 'contextmenu click', options, post_context_menu)
	}
